...
## [fase.12] — Propuestas PDF + evaluación IA en licitaciones (completado)

- [fase.12] Módulo backend `proposals/`: `PdfTextService` (extracción de texto vía pdf-parse 2.4.5/pdfjs-dist), `ProposalStorageService` (S3/MinIO con `@aws-sdk/client-s3`, desactivado si faltan `AWS_*` vars), `AiEvaluationService` (OpenRouter con `response_format: json_object` + fallback heurístico determinista).
- [fase.12] `POST /api/licitaciones/join` acepta multipart con PDF **obligatorio** (`proposal`, 10 MB en memoria); sin archivo devuelve 400 "La propuesta técnica en PDF es obligatoria"; valida mimetype PDF y persiste metadatos en el JSON `providers` (sin migración). No existe vía posterior para adjuntar el PDF.
- [fase.12] Transición OPEN→REVEALING dispara la evaluación IA de propuestas (descarga S3 → texto → score/criterios/resumen); `qualityScore` reutiliza el score IA para el UI existente.
- [fase.12] Cierre REVEALING→CLOSED: ganador por puntaje compuesto `50% precio + 50% IA` (price score = min/monto), desempate por menor monto, `winnerReason`/`priceScore`/`finalScore` en la entrada del ganador.
- [fase.12] `GET /api/licitaciones/:id/providers/:providerId/proposal` (descarga PDF, solo participante/organizador y tras el cierre de compromisos) y `POST /api/licitaciones/:id/evaluate` (re-evaluación manual post-revelación).
- [fase.12] `serialize` enmascara en OPEN: `amount`, `proposalFileName`, `proposalSize`, `aiScore`, `aiEvaluation`.
- [fase.12] Frontend: `apiFetchForm` multipart con retry de sesión; `joinLicitacionWithProposal` (PDF obligatorio), `fetchProposalBlobUrl`, `evaluateLicitacion`; campos de propuesta/IA en `types.ts`.
- [fase.12] Frontend: input PDF obligatorio en el detalle de licitación y en el modal del supplier dashboard (aviso "Debes adjuntar tu propuesta en PDF para ofertar"); fila de proveedor con descarga de propuesta + score IA; podio con score compuesto y motivo del ganador; botón "Re-evaluar propuestas" para el organizador.
- [fase.12] Calidad: backend `tsc --noEmit` ✓ y `nest build` ✓; frontend `tsc --noEmit` ✓ y `eslint` 0 errores ✓.

## [fase.11] — Datos reales y eliminación de mocks (completado)

- [fase.11] Seed data realista: 8 licitaciones peruanas (3 OPEN, 1 REVEALING, 2 CLOSED, 1 DRAFT, 1 OPEN adicional) con empresas reales (TechSolutions SAC, SeguridadCorp SAC, LimpiezaPro EIRL, etc.).
- [fase.11] Usuarios seed: 3 compradores + 6 proveedores + 2 analistas de factor + 1 admin. Credenciales: `Compras123!`, `Proveedor123!`, `Analista123!`, `ChangeMe123!`.
- [fase.11] Supplier dashboard: KPIs reales calculados desde datos del usuario (tasa de adjudicación, ofertas en curso, licitaciones ganadas, nivel dinámico). Eliminado `proveedorDemo`.
- [fase.11] Buyer dashboard: "Total publicadas" y "Ahorro estimado" calculados desde licitaciones reales del usuario. Eliminados valores hardcodeados.
- [fase.11] Landing page: Métricas (licitaciones, proveedores activos, ahorro promedio) calculadas desde la API en tiempo real. Eliminado `METRICS` hardcodeado.
- [fase.11] Mock data limpiado: solo 2 licitaciones fallback para errores de infraestructura. Eliminados `podio`, `credentials`, `rwaAsset`.
- [fase.11] Eliminado componente `is-rwa-card.tsx` y sección RWA del supplier dashboard.
- [fase.11] Calidad: backend `tsc --noEmit` ✓, 63/63 tests ✓; frontend `tsc --noEmit` ✓, 42/42 tests ✓, `eslint` 0 errores ✓.
...
## [fase.7.licitabien.swauth] — Auth completo (registro + recuperación + 2FA + switch libre) (completado)

- [fase.7.licitabien.swauth] Switch libre de persona: `persona.ts` reemplaza el mapeo rol→persona por constantes `DEFAULT_APP_ROUTE` + `PERSONA_ROUTES`; `app-shell` y `LicitabienNav` muestran **ambos paneles** + "Perfil y reputación" + "Mi cuenta" para cualquier sesión (sin derivar rol); sin sesión: "Iniciar sesión" + "Registrarse". Eliminado `unified-dashboard-view.tsx`; `/licitabien/dashboard` redirige a `DEFAULT_APP_ROUTE`; el botón "Registrarse" de la landing se oculta si hay sesión; el CTA con sesión va a `DEFAULT_APP_ROUTE`.
- [fase.7.licitabien.swauth] Modelo de usuario: `User` añade `phone`, `dni` (@unique), `recoveryQuestion`, `recoveryAnswerHash`, `totpSecret`, `totpEnabled`. Aplicado a Neon con `prisma db push` (el esquema remoto ya tenía drift de migraciones) y `prisma generate`.
- [fase.7.licitabien.swauth] Backend auth: DTOs `register.dto.ts` (teléfono 9, dni 8, contraseña reutiliza `PASSWORD_PATTERN`), `recovery.dto.ts` y `two-fa.dto.ts`. `AuthService` con `register` (crea ANALYST, hashea password y respuesta de recuperación con bcrypt, valida email/dni únicos), `recoveryInit` (devuelve pregunta; placeholder genérico para no filtrar), `recoveryReset` (verifica respuesta + nueva contraseña + `revokeAllUserSessions`), `setup2fa`/`confirm2fa`/`disable2fa` y `verify2fa`. `login` modificado: si `totpEnabled` **no** emite cookies → `{ step: 'verify-2fa', pendingToken }` (JWT 5 min); `verify2fa` completa la sesión real. `AuthUserView`, `AccessTokenPayload` y `AuthUser` añaden `totpEnabled`. Endpoints: `POST /auth/register`, `/auth/recovery/init`, `/auth/recovery/reset`, `/auth/2fa/setup`, `/auth/2fa/confirm`, `/auth/2fa/disable`, `/auth/login/verify-2fa` (todos `@Public` salvo los de 2FA que requieren sesión).
- [fase.7.licitabien.swauth] TOTP sin dependencias externas: `totp.ts` implementa HOTP/TOTP (RFC 6238), base32, ventana ±30s y URI `otpauth://` sobre `node:crypto`. El QR lo renderiza el frontend con su paquete `qrcode` ya presente (sin instalar `otplib`/`qrcode` en el backend — el `&` en la ruta UNC impedía `npm install` en este entorno).
- [fase.7.licitabien.swauth] Frontend auth: `lib/types.ts` añade `totpEnabled`; `lib/auth.ts` con `register`, `recoveryInit`, `recoveryReset`, `setup2fa`, `confirm2fa`, `disable2fa`, `verify2fa`, y `login` que devuelve `AuthUserView | TwoFaChallenge`. `IsAuthProvider` expone `verify2faLogin`. `login-form.tsx` con máquina de estados (credenciales → código de 6 dígitos), enlaces a "Regístrate" y "¿Olvidaste tu contraseña?", redirect por defecto a `DEFAULT_APP_ROUTE`. Páginas nuevas: `/register` + `register-form.tsx`, `/forgot-password` + `recovery-form.tsx` (email → pregunta → respuesta + contraseña), `/licitabien/cuenta` + `cuenta-view.tsx` (datos, cambio de contraseña, toggle 2FA con QR).
- [fase.7.licitabien.swauth] Regla anti-auto-oferta: `LicitacionesService.join()` rechaza (`VALIDATION_ERROR` 400) si `organizerId === userId`; el frontend (`supplier-dashboard-view.tsx`) deshabilita el botón de oferta y avisa cuando `organizerId === user.id`.
- [fase.7.licitabien.swauth] Seed: admin y analistas con `phone`, `dni`, `recoveryQuestion`, `recoveryAnswerHash` (respuesta `invoice123`); reseed idempotente contra Neon.
- [fase.7.licitabien.swauth] Tests: `totp.spec.ts` (generación, verificación, ventana ±30s, URI otpauth) y extensión de `auth.service.spec.ts` con `register` (éxito, conflictos email/dni), `recovery` (init/UI, reset ok/rechazo), `2FA` (setup, login desafío, confirm/disable). Mocks de `AuthUser` actualizados con `totpEnabled` en adapters/invoices/permissions.
- [fase.7.licitabien.swauth] Calidad: backend `tsc --noEmit` limpio y frontend `tsc --noEmit` + `eslint` limpios (la advertencia informativa "Pages directory cannot be found" de eslint-config-next no afecta el exit code). Notas del entorno WSL↔Windows: `npm install`/`vitest`/`tsx` (esbuild) fallan por ausencia de binarios nativos de la plataforma bajo rutas UNC con `&`; la verificación se hizo con `tsc --noEmit` y `eslint` directos.

## [fase.8.licitabien.onchain] — BlindBidVault on-chain conectado y funcional (sin demos)

- [fase.8.licitabien.onchain] Escrituras on-chain (backend `bidding.service.ts`): `recordAuditScore` ahora escribe `setAuditScore` on-chain (AUDITOR_ROLE — el deployer/operador ya lo tiene) antes del upsert del mirror (dev mode: solo espeja si no hay signer, con `warn`). Añadidos `autoReveal(auctionId,bidder)` (permisivo: firma `revealBid` con el `secretEncrypted` de la delegación y la marca `REVEALED`), `settleAuctionOnChain(auctionId)` (permisivo, sincroniza winner/winningPrice) y `slashBidOnChain(auctionId,bidder)` (permisivo, marca la delegación `FAILED`). `commitBid`/`claimRefund` se mantienen firmados por la wallet del usuario (regla #7: el backend nunca mezcla los USDC del operador); el backend calcula `commitment = keccak256(abi.encodePacked(price, secret))` y guarda la delegación cifrada.
- [fase.8.licitabien.onchain] API: `BiddingController` expone `POST /api/auctions/:id/reveal/:bidder`, `POST /api/auctions/:id/settle` y `POST /api/auctions/:id/slash/:bidder`. `endpoints.ts` (frontend) añade `auctionsApi.reveal/settle/slash`.
- [fase.8.licitabien.onchain] Hooks wagmi (`useBlindBidVault.ts`): añadidos `useHasCommitted(auctionId,bidder)` (read), `useCancelAuction`, `useSetAuditScore` (solo si la wallet tiene AUDITOR_ROLE), y archivo nuevo `useToken.ts` con ABI ERC20 mínima + `useTokenBalance`, `useAllowance`, `useApproveToken` (flujo USDC: approve(vault,stakeAmount) → commitBid ya existente). `ARBITRUM_TOKEN_DECIMALS=6` (Circle testnet USDC `0x75fa…aa4d`).
- [fase.8.licitabien.onchain] UI Licitabien: el hero de la landing (`HeroAuctionCard`) pasa del mock a la **primera subasta ACTIVA del espejo on-chain** (víctor del backend vía `useLiveAuction`/`auctionsApi.list`), con badge "SUBASTA ON-CHAIN #N" y fallback al mock si no hay ninguna activa o la API no responde. El panel Protocolo `/auctions/[id]` ya cubre commit→reveal→settle→claimRefund on-chain con la wallet; se reutilizan los hooks de tx (pending/confirmed/reverted) y enlaces Arbiscan (`chain.ts`).
- [fase.8.licitabien.onchain] Contratos — Tests Foundry `contracts/test/BlindBidVault.t.sol` (`MockUSDC` + `BlindBidVaultTest`): happy path commit-reveal → ganador por composite score (precio 70 + auditoría 30), griefing (no-reveal → `slashBid` al treasury, slashing confirmado y excluido de refund), refund post-settle al no-ganador, límites (`PriceOutOfBounds`, constructor con weights ≠ 100, commit tras `commitEnd`), roles (`setAuditScore` solo AUDITOR_ROLE, `cancelAuction` solo organizer) e invariante de conservación de USDC. Nota: `forge` no está instalado en este entorno WSL↔Windows; los tests se escribieron contra la ABI/errores del contrato pero no se ejecutaron aquí.
- [fase.8.licitabien.onchain] Tests backend: `bidding.service.spec.ts` (commitmentHash contra viem, `formatPrice`, validación 0–100 de `recordAuditScore`, modo dev sin signer, y `BLOCKCHAIN_NOT_CONFIGURED` para autoReveal/settle/slash; sanity del ABI). 
- [fase.8.licitabien.onchain] Calidad: backend y frontend `tsc --noEmit` limpios; frontend `eslint` limpio. Pendientes de decisión (no bloquean, marcados en el plan): destino del módulo licitaciones demo (¿eliminar o mantener como fallback de lectura sin RPC?), commit custodial vs wallet (elegido: wallet + hash en servidor) y dónde persisten título/metadata de la subasta (hoy en la tabla `Auction` con `title`/`description`). Smoke E2E en vivo (crear primera subasta real en Sepolia y ciclo completo) requiere financiar el signer del backend y wallets de bidders con USDC testnet (faucet.circle.com) — no ejecutado en este entorno.

## [fase.9.licitabien.fork] — Frontend cableado al fork local de Anvil (Arbitrum Sepolia)

- [fase.9.licitabien.fork] **Red local**: `anvil --fork-url https://sepolia-rollup.arbitrum.io/rpc` corre con **chainId 421614** (Arbitrum Sepolia, confirmado con `cast chain-id` y `eth_chainId`). Se usa la chain `arbitrumSepolia` existente en `wagmi.ts` (no hace falta una custom 31337); el transporte ya lee `NEXT_PUBLIC_ARBITRUM_SEPOLIA_RPC_URL`, así que `.env.local` apunta ese var a `http://127.0.0.1:8545`.
- [fase.9.licitabien.fork] **Direcciones**: `addresses.ts` + `.env.local`/`.env.example` actualizan `BlindBidVault` a `0x95Ba1d95AaB9AE9cEE86b676b1054566eD636597` (código verificado on-chain con `cast code`) y exponen `EAS_SCHEMA_UID = 0x4fa9cf44…d5f5`.
- [fase.9.licitabien.fork] **Wallets**: `Web3Provider.tsx` añade `import '@rainbow-me/rainbowkit/styles.css'` (faltaba; el modal RainbowKit no renderizaba estilos). Las cuentas de prueba de Anvil se importan en MetaMask (clave privada) — `WalletButton`/`ConnectButton` ya estaba cableado en `providers.tsx`→`layout.tsx`.
- [fase.9.licitabien.fork] **Hashing ciego**: nuevo `lib/web3/commitment.ts` con `generateCommitHash(price: bigint, secret: string)` = `keccak256(abi.encodePacked(uint256, string))` vía `viem.encodePacked`. El detalle de subasta (`app/(app)/auctions/[id]/page.tsx`) deja de hashear inline (violaba la regla "lógica blockchain fuera de componentes") y llama a la utilidad. Tests `commitment.test.ts` con **vectores cruzados contra Foundry** (`cast keccak` sobre el packed): 100 USDC/`"secret"` → `0xe2c0…917b`, 1 USDC/`""` → `0xc1af…7e53`, 100/`"secret"` → `0xb2b2…a684`.
- [fase.9.licitabien.fork] **Read hook**: confirmado `useAuction(auctionId)` (`useBlindBidVault.ts`) ya apunta a `auctions(uint256)` con la dirección/ABI correctas y `query.enabled` defensivo.
- [fase.9.licitabien.fork] **Deuda técnica saldada**: arreglados errores preexistentes de typecheck/lint que bloqueaban CI — `bindWallet` ahora retorna `AuthUser` (era `User`, forzaba `as any`); `reputation-view.tsx` extrae `.data` del envelope `ApiResult` y tipa `Credential` correctamente (antes resolvía al `Credential` del DOM lib); `permissions.test.ts` añade `walletAddress` a los fixtures; `variant="outline"` → `secondary` (no existía); imports sin uso eliminados.
- [fase.9.licitabien.fork] Calidad: frontend `npm run test` 36/36 ✓, `tsc --noEmit` limpio ✓, `eslint` 0 errores (1 warning de `<img>` preexistente) ✓.

## [fase.10.licitabien.approve] — Flujo Approve USDC + Respaldo secreto en localStorage (completado)

- [fase.10.licitabien.approve] **Dirección en testnet pública**: `BlindBidVault` actualizado a `0xe582DAa8293b664c74BfC29ADd2c47eB96471b2b` (Arbitrum Sepolia). `addresses.ts`, `.env.local` y `.env.example` sincronizados.
- [fase.10.licitabien.approve] **Constante USDC**: añadida dirección `0x75faf114eafb1BDbe2F0316DF893fd58CE46AA4d` (Circle testnet USDC) a `contractAddresses` con env override `NEXT_PUBLIC_USDC_ADDRESS_SEPOLIA` (recomendación: constante hardcodeada, 1 read menos a la blockchain).
- [fase.10.licitabien.approve] **Flujo de aprobación ERC-20**: en el form de commit (`app/(app)/auctions/[id]/page.tsx`), se usa `useAllowance(usdcAddress, address, vaultAddress)` + `useApproveToken(usdcAddress)` (hooks preexistentes en `useToken.ts`). Si `allowance < stakeAmount`, se renderiza botón **"Aprobar USDC"** (`onClick → approveHook.approve(vaultAddress, stake)`); tras confirmación, wagmi refresca allowance por bloque y el botón cambia a **"Enviar Puja Ciega"** (submit). UX de carga: `loading = allowance.isLoading || approveHook.isPending || approveHook.isConfirming`.
- [fase.10.licitabien.approve] **Respaldo del secreto** (localStorage): nuevo `lib/web3/bid-storage.ts` con `saveBidSecret(auctionId, {price,secret})`, `getBidSecret(auctionId)`, `clearBidSecret(auctionId)` — SSR-safe, robusto ante JSON corrupto, clave `licitabien_bid_${auctionId}`. Cuando `commitConfirmed` (transición detectada en render via patrón React 19 "adjusting state"), guarda `{price, secret}`. Al cargar/montar, lee `getBidSecret(bigId)` y autocompleta **ambos forms** (Manual reveal + Delegación a agente IA) para proteger contra slashing por olvido humano. Tras `revealConfirmed`, limpia el secreto.
- [fase.10.licitabien.approve] Tests: `bid-storage.test.ts` (6 tests — save/get/clear, aislamiento por auctionId, JSON corrupto, estructura inválida). Total frontend: 42/42 ✓.
- [fase.10.licitabien.approve] Calidad: `npm run test` 42/42 ✓, `tsc --noEmit` limpio ✓, `eslint` 0 errores (1 warning preexistente) ✓. Adaptado al linter estricto de Next.js 16 (sin setState en effects, sin refs en render).
